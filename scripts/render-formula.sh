#!/usr/bin/env bash
# Render packaging/homebrew/burnbox.rb.tmpl into a concrete Formula by
# substituting VERSION + per-platform SHA256s from a checksums file
# (lines of "<sha>  <file>", as produced by `make release-local`).
#
...
SUMS="${2:?checksums file required}"
OUT="${3:?output path required}"

repo_root=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)
tmpl="${repo_root}/packaging/homebrew/burnbox.rb.tmpl"
[ -f "$tmpl" ] || { echo "missing template: $tmpl" >&2; exit 1; }
[ -f "$SUMS" ] || { echo "missing checksums: $SUMS" >&2; exit 1; }

# Look up the sha for a given "<os>-<arch>" asset.
lookup() {
    local asset="burnbox-${VERSION}-$1.tar.gz"
    local sha
...
echo "✓ rendered $OUT (burnbox $VERSION)"
